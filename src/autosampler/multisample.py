...
def make_sfz(
    *,
    ...
    lines: list[str] = [
        "// Generated by ARP 2600 AutoSampler",
        "",
    ]

    for zone in zones:
        # Basic SFZ opcodes supported by nearly every SFZ reader.
        lines.append(
            " ".join(
                [
                    "<region>",
                    f"sample=Samples/{zone.filename}",
                    f"pitch_keycenter={zone.root}",
                    f"lokey={zone.key_low}",
                    f"hikey={zone.key_high}",
                    f"lovel={zone.vel_low}",
                    f"hivel={zone.vel_high}",
                ]
            )
        )
